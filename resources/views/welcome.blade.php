...
<div class="container pt-5">
    <div class="row">
                    ...
                    <div class="col-6 col-md-6 col-lg-4 mb-3">
                        <div class="card h-100 border-0">
                            ...
                            <button type="button" class="btn btn-success add-product-button" data-id='{{ $product->id }}' @guest disabled @endguest>
                                <i class="fa-solid fa-cart-plus"></i> {{__('shop.cart.add_item')}}
                            </button>
                        </div>
                    </div>
                    @endforeach
        ...
        <form class="col-md-4 order-md-1 col-lg-3 sidebar-filter">
            ...
            <h6 class="text-uppercase font-weight-bold mb-3">{{__('shop.product.fields.categories')}}</h6>
            @foreach ($categories as $category)
            <div class="mt-2 mb-2 pl-2">
                <div class="custom-control custom-checkbox">
                    <input type="checkbox" class="custom-control-input" name="filter[categories][]" value="{{$category->id}}" id="category-{{$category->id}}">
                    <label class="custom-control-label" for="category-{{$category->id}}">{{$category->name}}</label>
                </div>
            </div>
            @endforeach
            <div class="divider mt-5 mb-5 border-bottom border-secondary"></div>
            <h6 class="text-uppercase mt-5 mb-3 font-weight-bold">{{__('shop.product.fields.price')}}</h6>
